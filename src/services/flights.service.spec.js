...
describe("[Unit] That Flights Service", () => {
  ...
  it("get correct flight by id with correct parsers", () => {
    ...
    const flight = flightsService.getFlightById(
      "FROM",
      "TO",
      new Date(2018, 9, 30),
      1
    ...
    expect(flight).toHaveProperty(
      "departDate",
      expect.stringMatching(/\w+ \d{1,2}\w{2} \d{4}/)
    );
    ...
    expect(flight.segments[0]).toHaveProperty(
      "departTime",
      expect.stringMatching(/\d{2}:\d{2} [AP]M/)
    );
    expect(flight.segments[0]).toHaveProperty(
      "arrivalTime",
      expect.stringMatching(/\d{2}:\d{2} [AP]M/)
